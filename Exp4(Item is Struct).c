...
{
  /* USER CODE BEGIN 5 */
	Data dataSend01={12,1};
  for(;;)
  {
		
		osMessagePut(Queue01Handle,(uint32_t)&(dataSend01),200);
		printf("%d:\tTask1(sender01):\tValue sent:%d\n",
						HAL_GetTick(),
						dataSend01.value);
		osDelay(2000);
  }
...
void StartReceiver(void const * argument)
{
  ...
  for(;;)
  {
		receiValue=osMessageGet(Queue01Handle,4000);
		printf("%d:\tTask3(Receiver):\tValue received:%d from Source:%d\n",
						HAL_GetTick(),
						((Data*)receiValue.value.p)->value,
						((Data*)receiValue.value.p)->source);
  }
...
void StartSender02(void const * argument)
{
  ...
  /* Infinite loop */
	Data dataSend02={13,2};
  for(;;)
  {
    osMessagePut(Queue01Handle,(uint32_t)&(dataSend02),200);
		printf("%d:\tTask2(sender02):\tValue sent:%d\n",
						HAL_GetTick(),
						dataSend02.value);
		osDelay(2000);
  }
