...
{
  /* USER CODE BEGIN 5 */
	uint32_t value1Send=12;
  /* Infinite loop */
  for(;;)
  {
		
		osMessagePut(Queue01Handle,value1Send,200);
...
void StartSender02(void const * argument)
{
  ...
  /* Infinite loop */
	...
	uint32_t valueSend02=13;
  for(;;)
  {
		
		osMessagePut(Queue01Handle,valueSend02,200);
...
void StartReceiver(void const * argument)
{
  ...
  for(;;)
  {
		receiveValue=osMessageGet(Queue01Handle,4000);
		printf("%d:\tTask3(receiver):\tValue received:%d\n ",
						HAL_GetTick(),
						receiveValue.value.v);
  }
